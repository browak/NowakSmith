...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{tokenList} \hlkwb{<-} \hlstd{bigramCount}\hlopt{$}\hlstd{ngrams}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in \{: task 1 failed - "{}object 'text2' not found"{}}}\begin{alltt}
\hlkwd{head}\hlstd{(}\hlkwd{colnames}\hlstd{(M))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in is.data.frame(x): object 'M' not found}}\begin{alltt}
\hlkwd{str}\hlstd{(M)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in str(M): object 'M' not found}}\end{kframe}
\end{knitrout}
